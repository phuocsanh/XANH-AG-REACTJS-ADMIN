...
export function mapApiResponseToInventoryReceipt(
  ...
  return {
    ...
    // Payment fields
    paid_amount: apiReceipt.paid_amount
      ? parseFloat(apiReceipt.paid_amount)
      : 0,
    ...
    adjusted_amount: apiReceipt.adjusted_amount
      ? parseFloat(apiReceipt.adjusted_amount)
      : 0,
    returned_amount: apiReceipt.returned_amount
      ? parseFloat(apiReceipt.returned_amount)
      : 0,
    final_amount: apiReceipt.final_amount
      ? parseFloat(apiReceipt.final_amount)
      : undefined,
    supplier_amount: apiReceipt.supplier_amount
      ? parseFloat(apiReceipt.supplier_amount)
      : undefined,
...
export function mapApiResponseToInventoryReceiptItem(
  ...
  return {
    ...
    product_name:
      apiItem.product?.trade_name ||
      apiItem.product?.name ||
      apiItem.product_name ||
      "",
    unit_name:
      apiItem.product?.unit?.name ||
      apiItem.product?.unit_name ||
      apiItem.unit_name ||
      "",
...
export const normalizeReceiptStatus = (status: any): InventoryReceiptStatus => {
  ...
  if (
    ...
    return InventoryReceiptStatus.APPROVED
...
export const getInventoryTransactionTypeText = (
...
export interface UpdateInventoryReceiptRequest
  extends Partial<CreateInventoryReceiptRequest>, AnyObject {
  id: number // ID của phiếu nhập cần cập nhật
}

// Interface cho form chi tiết phiếu nhập hàng
export interface InventoryReceiptItemForm extends CreateInventoryReceiptItemRequest {
  key: string
  product_name?: string
...
export interface UpdateInventoryReceiptItemRequest
  extends Partial<CreateInventoryReceiptItemRequest>, AnyObject {
