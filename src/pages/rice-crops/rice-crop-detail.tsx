...
import { Tabs, Card, Descriptions, Tag, Button, Spin, Breadcrumb } from "antd"
import {
  ArrowLeftOutlined,
  EditOutlined,
  HomeOutlined,
...
const RiceCropDetail: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState("profit") // Mặc định là tab Báo cáo lợi nhuận nông dân (key: 'profit')
  ...
  if (!riceCrop) {
    return (
      <div className='p-6 text-center'>
        ...
        <Button onClick={() => navigate("/rice-crops")}>
          Quay lại danh sách
        </Button>
      </div>
    )
  ...
  const detailTabItems = [
    {
      ...
      children: (
        <ProfitReportTab
          ...
          amountOfLand={Number(riceCrop.amount_of_land)}
        />
      ),
    },
    {
          ...
          >
            ...
            <Descriptions.Item label='Giai đoạn'>
              <Tag
                color={
                  growthStageColors[
                    riceCrop.growth_stage as keyof typeof growthStageColors
                  ]
                }
              >
                {
                  growthStageLabels[
                    riceCrop.growth_stage as keyof typeof growthStageLabels
                  ]
                }
              </Tag>
            </Descriptions.Item>
            <Descriptions.Item label='Trạng thái'>
              <Tag
                color={
                  statusColors[riceCrop.status as keyof typeof statusColors]
                }
              >
            ...
            <Descriptions.Item label='Ngày gieo'>
              <div className='flex flex-col'>
                <div className='flex items-center gap-2 flex-wrap'>
                  <span style={{ color: "#000", fontWeight: "bold" }}>
                    {riceCrop.sowing_date
                      ? dayjs(riceCrop.sowing_date).format("DD/MM/YYYY")
                      : "-"}
                  </span>
                  {String(riceCrop.status).toLowerCase().includes("active") &&
                    riceCrop.sowing_date && (
                      <Tag
                        color='success'
                        className='m-0 text-[11px] font-bold border-green-200 whitespace-nowrap'
                      >
                ...
                {riceCrop.sowing_lunar_date && (
                  <span className='text-[11px] text-[#2c7a32] font-semibold'>
                    {riceCrop.sowing_lunar_date}
                  </span>
                )}
              </div>
                ...
                <div className='flex items-center gap-2 flex-wrap'>
                  <span style={{ color: "#000", fontWeight: "bold" }}>
                    {riceCrop.transplanting_date
                      ? dayjs(riceCrop.transplanting_date).format("DD/MM/YYYY")
                      : "-"}
                  </span>
                  {String(riceCrop.status).toLowerCase().includes("active") &&
                    riceCrop.transplanting_date && (
                      <Tag
                        color='cyan'
                        className='m-0 text-[11px] font-bold border-cyan-200 whitespace-nowrap'
                      >
                        {calculateDaysDiff(riceCrop.transplanting_date)} ngày
                        sau cấy
                      </Tag>
                    )}
                </div>
              ...
              <div className='flex flex-col'>
                <span style={{ color: "#000" }}>
                  {riceCrop.expected_harvest_date
                    ? dayjs(riceCrop.expected_harvest_date).format("DD/MM/YYYY")
                    : "-"}
                </span>
            ...
            <Descriptions.Item label='Ngày thu hoạch thực tế'>
              <span style={{ color: "#000" }}>
                {riceCrop.actual_harvest_date
                  ? dayjs(riceCrop.actual_harvest_date).format("DD/MM/YYYY")
                  : "-"}
              </span>
            </Descriptions.Item>
      ...
      ? [
          {
            ...
            children: (
              <FarmServiceCostTab
                riceCropId={riceCrop.id}
                customerId={riceCrop.customer_id}
              />
            ),
          },
        ]
  ...
  return (
    <div className='p-6'>
      ...
      <div className='flex flex-wrap items-center gap-2 mb-6'>
        ...
        <Tag
          color={statusColors[riceCrop.status as keyof typeof statusColors]}
          className='m-0 text-sm sm:text-base py-0.5 px-2'
        >
          {statusLabels[riceCrop.status as keyof typeof statusLabels]}
        </Tag>
      </div>
