...
const RiceCropsList: React.FC = () => {
  ...
  const seasonOptions = React.useMemo(
    () =>
      seasonsData?.data?.items?.map((s: any) => ({
        label: s.name,
        value: s.id,
      })) || [],
    [seasonsData],
  ...
  const columns = [
    // Chỉ hiển thị cột Khách hàng nếu không phải CUSTOMER - đặt kế cột STT
    ...(!isCustomer
      ? [
          {
            key: "customer_name",
    ...
    },
    {
      ...
      title: "Giai đoạn",
      width: 150,
      filters: Object.entries(growthStageLabels).map(([value, text]) => ({
        text,
        value,
      })),
    ...
    },
    {
      ...
      render: (record: ExtendedRiceCrop) => (
        <div>
          {record.sowing_date
            ? dayjs(record.sowing_date).format("DD/MM/YYYY")
            : "-"}
        </div>
      ),
    },
