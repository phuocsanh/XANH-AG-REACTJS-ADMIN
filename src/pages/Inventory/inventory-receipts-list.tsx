...
const InventoryReceiptsList: React.FC = () => {
  ...
  const renderStatus = (status: InventoryReceiptStatus, statusText: string) => {
    const statusConfig = {
      [InventoryReceiptStatus.DRAFT]: { color: 'default', icon: <EditOutlined /> },
      [InventoryReceiptStatus.PENDING]: { color: 'processing', icon: <SyncOutlined spin /> },
      [InventoryReceiptStatus.APPROVED]: { color: 'success', icon: <CheckOutlined /> },
      [InventoryReceiptStatus.COMPLETED]: { color: 'success', icon: <CheckOutlined /> },
      [InventoryReceiptStatus.CANCELLED]: { color: 'error', icon: <CloseOutlined /> }
    }
  ...
  const renderActions = (record: InventoryReceipt) => {
    ...
    if (record.status === InventoryReceiptStatus.DRAFT || 
        record.status === InventoryReceiptStatus.PENDING) {
      actions.push(
    ...
    if (record.status === InventoryReceiptStatus.PENDING) {
      actions.push(
    ...
    if (record.status === InventoryReceiptStatus.APPROVED) {
      actions.push(
    ...
    if (record.status === InventoryReceiptStatus.DRAFT || 
        record.status === InventoryReceiptStatus.PENDING ||
        record.status === InventoryReceiptStatus.APPROVED) {
      actions.push(
    ...
    if (record.status === InventoryReceiptStatus.DRAFT || 
        record.status === InventoryReceiptStatus.CANCELLED) {
      actions.push(
  ...
  const columns: ColumnsType<InventoryReceipt> = [
    ...
    },
    {
      ...
      key: 'supplierName',
      width: 200,
      render: (supplierName: string) => supplierName || '-'
    },
    {
      title: 'Tổng tiền',
    ...
    },
    {
      ...
      width: 120,
      align: 'center',
      render: (status: InventoryReceiptStatus, record: InventoryReceipt) =>
        renderStatus(status, record.statusText)
    },
    {
      title: 'Ngày tạo',
      dataIndex: 'createdAt',
        ...
        <Row gutter={[16, 16]}>
          ...
          <Col xs={24} sm={12} md={6}>
            <Select
              ...
              style={{ width: '100%' }}
            >
              <Select.Option value={InventoryReceiptStatus.DRAFT}>Nháp</Select.Option>
              <Select.Option value={InventoryReceiptStatus.PENDING}>Chờ duyệt</Select.Option>
              <Select.Option value={InventoryReceiptStatus.APPROVED}>Đã duyệt</Select.Option>
              <Select.Option value={InventoryReceiptStatus.COMPLETED}>Hoàn thành</Select.Option>
              <Select.Option value={InventoryReceiptStatus.CANCELLED}>Đã hủy</Select.Option>
            </Select>
          </Col>
          <Col xs={24} sm={12} md={6}>
            <Input
