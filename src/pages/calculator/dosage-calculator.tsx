  ...
  return (
    <div className='p-2 md:p-6'>
      <Card
        title={
          <>
            <CalculatorOutlined /> Tính Liều Lượng Thuốc
          </>
        }
        className='shadow-md'
        bordered={false}
...
const AreaCalculator: React.FC = () => {
  ...
  const handleValuesChange = (
    changedValues: Partial<typeof values>,
    ...
    if ("congCount" in changedValues || "congType" in changedValues) {
      ...
      const cCount =
        "congCount" in changedValues
          ? Number(changedValues.congCount)
          : values.congCount
      const cType =
        "congType" in changedValues ? changedValues.congType : values.congType
      ...
      if (unit) {
        ...
        const areaUnitVal = Object.values(AREA_UNITS).find(
          (u) => u.value === values.areaUnit,
        )
        if (areaUnitVal) {
      ...
      const aInput =
        "areaInput" in changedValues
          ? Number(changedValues.areaInput)
          : values.areaInput
      const aUnit =
        "areaUnit" in changedValues ? changedValues.areaUnit : values.areaUnit
      ...
      if (unit) {
        ...
        // Update Cong Count
        const congUnitVal = Object.values(AREA_UNITS).find(
          (u) => u.value === values.congType,
        )
        if (congUnitVal) {
    ...
    const newState = {
      ...
      congCount:
        "congCount" in changedValues
          ? Number(changedValues.congCount)
          : newCongCount, // Ensure we use the exact input if user typed it
      areaInput:
        "areaInput" in changedValues
          ? Number(changedValues.areaInput)
          : newAreaInput,
      ...
      <Col xs={24} md={12}>
        ...
        >
          <Title level={5} className='mb-4'>
            1. Thông tin quy định (trên chai thuốc)
          </Title>

          <Row gutter={10}>
            <Col span={14}>
              <Form.Item label='Liều lượng khuyến cáo' name='dosageRate'>
                <NumberInput placeholder='VD: 1.5' size='middle' />
              </Form.Item>
            </Col>
            <Col span={10}>
          ...
          <Row gutter={10}>
            <Col span={14}>
              <Form.Item label='C2: Diện tích tổng' name='areaInput'>
                <NumberInput placeholder='VD: 10000' size='middle' />
              </Form.Item>
            </Col>
            <Col span={10}>
              ...
              {result ? (
                <>
                  <Text className='block text-gray-500 mb-2'>
                    ...
                    (tương đương {values.totalAreaM2.toLocaleString("vi-VN")}{" "}
                    m2) là:
                  </Text>
                  <Statistic
                    value={result.amount}
            ...
            {result && (
              <div className='bg-white p-4 rounded-lg border border-blue-100 text-left'>
                ...
                <ul className='list-disc pl-5 mt-2 text-gray-600'>
                  <li>
                    Diện tích (m2) ={" "}
                    {values.totalAreaM2.toLocaleString("vi-VN")} m2
                  </li>
                  <li>
                    Quy đổi diện tích về Hecta:{" "}
                    {(values.totalAreaM2 / 10000).toLocaleString("vi-VN", {
                      maximumFractionDigits: 4,
                    })}{" "}
                    ha
                  </li>
                  <li>
                    Lấy liều lượng khuyến cáo (trên 1 ha) nhân với diện tích
                    (ha).
                  </li>
                  <li>
                    Kết quả: <b>{result.display}</b>
                  </li>
                </ul>
              </div>
            )}
...
const RatioCalculator: React.FC = () => {
  ...
  const calculate = (vals: typeof values) => {
    ...
    const resultAmount =
      (Number(chemAmount) / Number(targetAmount)) * Number(actualAmount)
  ...
  return (
    <Row gutter={[16, 16]}>
      <Col xs={24} md={12}>
        ...
        >
          <Title level={5} className='mb-4'>
            1. Tỷ lệ pha chuẩn (Theo hướng dẫn)
          </Title>
          <Text type='secondary' className='mb-2 block'>
            Ví dụ: Pha <b className='text-black'>20ml</b> thuốc cho{" "}
            <b className='text-black'>10kg</b> giống.
          </Text>
          ...
          <div className='flex justify-center items-center my-2'>
            <Text strong className='text-xl text-gray-400'>
              DÙNG CHO
            </Text>
          </div>
          ...
          <Form.Item
            label={`Tổng lượng ${values.targetUnit === "l" ? "nước" : "giống"} cần xử lý`}
            name='actualAmount'
          >
            <NumberInput
              placeholder='VD: 400'
              size='middle'
              addonAfter={values.targetUnit}
            />
          </Form.Item>
        </Form>
      </Col>
          ...
          <div className='text-center'>
                ...
                <>
                  <Text className='block text-gray-500 mb-2'>
                    ...
                    <b className='text-gray-800'>
                      {values.actualAmount} {values.targetUnit}
                    </b>{" "}
                    giống/nước, bạn cần lượng thuốc là:
                  </Text>
                  <Statistic
                    value={result.amount}
                    precision={2}
            ...
            {result && (
              <div className='bg-white p-4 rounded-lg border border-blue-100 text-left'>
                ...
                <ul className='list-disc pl-5 mt-2 text-gray-600'>
                  <li>
                    Tỷ lệ: {values.chemAmount} {values.chemUnit} /{" "}
                    {values.targetAmount} {values.targetUnit}
                  </li>
                  <li>
                    Công thức: ({values.chemAmount} / {values.targetAmount}) x{" "}
                    {values.actualAmount}
                  </li>
                  <li>
                    Kết quả: <b>{result.display}</b>
...
const WaterBasedCalculator: React.FC = () => {
  ...
  const calculate = (vals: typeof values) => {
    ...
    if (customAreaAmount > 0) {
      const unit = Object.values(AREA_UNITS).find(
        (u) => u.value === customAreaUnit,
      )
      if (unit) {
    ...
    <Row gutter={[16, 16]}>
      <Col xs={24} md={12}>
        ...
        >
          ...
          <Text type='secondary' className='mb-2 block'>
            Ví dụ: <b className='text-black'>10ml</b> Litosen cho{" "}
            <b className='text-black'>10 lít</b> nước.
          </Text>
          ...
          <Title level={5} className='mb-4'>
            2. Lượng nước cần cho 10.000 m² (1 ha chuẩn)
          </Title>
          <Form.Item
            label='Lượng nước phun/tưới cho 10.000 m²'
            name='waterPerHa'
          >
          ...
          <Title level={5} className='mb-4'>
            3. Diện tích tùy chỉnh (Tùy chọn)
          </Title>
          ...
          {values.customAreaAmount > 0 && (
            <div className='mb-4 p-3 bg-blue-50 rounded border border-blue-200'>
              ...
              <Text className='block text-xs text-gray-600'>
                {values.customAreaAmount}{" "}
                {
                  Object.values(AREA_UNITS).find(
                    (u) => u.value === values.customAreaUnit,
                  )?.label
                }
                {" = "}
                {(
              ...
              {results ? (
                <>
                        ...
                        <Statistic
                          title={
                            <span className='text-base'>
                              Lượng thuốc cho 10.000 m²
                            </span>
                          }
                          value={results.perHa}
                          precision={2}
                          suffix={
                            <span className='text-lg font-bold ml-1'>ml</span>
                          }
                          valueStyle={{
                            color: "#1890ff",
                            fontSize: "2rem",
                  ...
                  <div className='bg-white p-4 rounded-lg border border-blue-100 text-left'>
                    ...
                    <ul className='list-disc pl-5 mt-2 text-gray-600'>
                      <li>
                        Tỷ lệ pha: {values.chemAmount}ml / {values.waterAmount}
                        lít ={" "}
                        {(values.chemAmount / values.waterAmount).toFixed(2)}{" "}
                        ml/lít
                      </li>
                      <li>Lượng nước cho 10.000 m²: {values.waterPerHa} lít</li>
                      <li>
                        Lượng thuốc cho 10.000 m²:{" "}
                        {(values.chemAmount / values.waterAmount).toFixed(2)} ×{" "}
                        {values.waterPerHa} ={" "}
                        <b>{formatAmount(results.perHa)}</b>
                      </li>
                    </ul>
                  </div>
                </>
