...
import { Button, Form, Input, message, Modal, Select, Table, Tag, Image, Space, Popconfirm, Descriptions } from 'antd';
import { PlusOutlined, SearchOutlined, FileSearchOutlined, EyeOutlined, EditOutlined, DeleteOutlined } from '@ant-design/icons';
    ...
    <div className="p-4">
      ...
      {!displayProducts.length ? (
        <div className="text-center py-12 bg-gray-50 rounded">
          <FileSearchOutlined style={{ fontSize: '48px', color: '#999', marginBottom: '16px' }} />
      ...
      <Modal
        title={currentProduct ? 'Cập nhật sản phẩm' : 'Thêm sản phẩm mới'}
        open={isModalVisible}
              ...
              {currentProduct.attributes && (
                <div>
                  ...
                  {currentProduct.attributes.height !== undefined && (
                    <div>Chiều cao: {String(currentProduct.attributes.height)}</div>
                  )}
                  {currentProduct.attributes.care_level !== undefined && (
                    <div>Mức độ chăm sóc: {String(currentProduct.attributes.care_level)}</div>
                  )}
                  <div>Có kèm chậu: {currentProduct.attributes.pot_included ? 'Có' : 'Không'}</div>
                </div>
              )}
            </Descriptions.Item>
          </Descriptions>
